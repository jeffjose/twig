...
mod tests {
    ...
    fn setup_test_repo() -> tempfile::TempDir {
        ...
        Command::new("git").args(&["add", "test.txt"]).output().unwrap();
        Command::new("git").args(&["commit", "-m", "Initial commit"]).output().unwrap();
    ...
    fn test_git_basic() {
        ...
        assert_eq!(result, "main"); // or "master" depending on git version
    ...
    fn test_git_combined_status() {
        ...
        Command::new("git").args(&["add", "test.txt"]).output().unwrap(); // stage change
        ...
        let result = GitProvider::get_value(&config).unwrap();
        assert!(result.contains("main"));    // branch name
        assert!(result.contains("+1"));      // staged changes
